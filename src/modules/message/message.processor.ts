...
interface JobMap {
  ...
  updateStatus: { data: UpdateStatusData; result: { messageId?: string; status: string } };
}

type JobName = keyof JobMap;

type Handler<K extends JobName> = (data: JobMap[K]['data']) => Promise<JobMap[K]['result']>;
...
export class MessageProcessor extends WorkerHost {
  ...
  private readonly handlers: HandlerMap = {
    saveMessage: async ({ chatId, content, sender }) => {
      const message = await this.service.saveMsg(chatId, content, sender);
  ...
  async process(job: Job<
    JobMap[JobName]['data'], JobMap[JobName]['result'], JobName
